...
- [x] **Measured speedups** (unchecked+rayon, 1920x1080 and 3840x2160 AVIF):
  ...
  - ASM+rayon 4K 8-tile: 23.1ms (1.13× vs ASM-1T 26.0ms)
...
- [ ] Replace PicOffset with TilePlaneRows in recon_b_intra/recon_b_inter
  - **Blocker**: PicOffset uses absolute frame coordinates + frame-width stride.
    Per-tile buffers can't be SB-row-sized because `4*(t.b.y * stride + t.b.x)`
    overflows. Frame-sized per-tile buffers waste 192MB for 4K 8-tile.
    The ONLY solution: replace PicOffset with row slices (no stride arithmetic).
    This requires changing ~50 DSP call sites in recon.rs from `.call(PicOffset)`
    to `.call_rows(&mut [&mut [BD::Pixel]], col_offset)`.
  - **Working fallback**: force_scalar disables SIMD on tile threads, scalar uses
    per-row guards. 1.8× on 4K 4-tile checked mode. Full SIMD with unchecked.
...
**NEVER CREATE UNSOUND ABSTRACTIONS. NEVER NEVER NEVER NEVER NEVER.**
**NEVER CREATE UNSOUND ABSTRACTIONS. NEVER NEVER NEVER NEVER NEVER.**
**NEVER CREATE UNSOUND ABSTRACTIONS. NEVER NEVER NEVER NEVER NEVER.**
**NEVER CREATE UNSOUND ABSTRACTIONS. NEVER NEVER NEVER NEVER NEVER.**
**NEVER CREATE UNSOUND ABSTRACTIONS. NEVER NEVER NEVER NEVER NEVER.**
...
The ONLY sound path to eliminating DisjointMut overlap for tile parallelism:
**Replace `&mut [u8] + offset + stride` with `&mut [&mut [BD::Pixel]]` (per-row slices) in SIMD dispatch.**
Each row is independently borrowed. Tiles get disjoint column ranges via `split_at_mut`.
No stride gaps. No contiguous slice over untracked memory. No shortcuts. No "strided guards."
The SIMD inner functions must change their interface. ~50 functions across safe_simd/.
